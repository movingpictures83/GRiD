...
input <- function(inputfile0) {
	print(inputfile0)
 pfix <- prefix()
   parameters <- readParameters(inputfile0)
   inputfile <<- paste(pfix, parameters["inputfile", 2], sep="/")
   inputfile2 <<- paste(pfix, parameters["inputfile2", 2], sep="/")
   inputfile3 <<- paste(pfix, parameters["inputfile3", 2], sep="/")
   #image <<- paste(pfix, parameters["image", 2], sep="/")
   #prefix <<- paste(pfix, parameters["prefix", 2], sep="/")
   coverage <<- as.numeric(parameters["coverage", 2])
   dnaa <<- as.numeric(parameters["dnaa", 2])
   dif <<- as.numeric(parameters["dif", 2])
}
...
title<-strapplyc(paste(outputfile,"txt",sep="."), "(.*)....", simplify = TRUE)
df[,-1] <- remove_outliers(df$mean)
df[, -1] <- lapply( df[, -1], function(x){ (x/sum(x, na.rm=TRUE))*100} )
df[, -1] <- log2(df$mean)
P <- ggplot(data = df, aes(x = x, y = mean))
...
GRiD_CI <- (1.96*(sd(as.numeric(unlist(GRiD_data)))))/sqrt(nrow(GRiD_data))
GRiD_data <- as.numeric(unlist(GRiD_data))
...
write(output_results, file = paste(outputfile, "txt", sep="."))
